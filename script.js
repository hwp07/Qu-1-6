...
function tapLove(id) {
  ...
  if (loveTaps.size === 4) {
    Swal.fire({
      ...
      typeWriterEffect(
        `Chúc ${userName} 1/6 thật vui vẻ như một đứa trẻ, được yêu thương như một nữ hoàng/hoàng tử 👑. Dù có lớn bao nhiêu thì vẫn luôn được cưng chiều mỗi ngày! 💘`,
        'loveMsg',
        () => {
          const fromTag = document.createElement("div");
          fromTag.id = 'fromTag';
          fromTag.textContent = "From: Hphong ";
...
async function inipesan() {
  const { value: typedName } = await Swal.fire({
    title: 'Nhập Tên Của Bạn Iuu Đee <3',
    input: 'text',
    inputValue: '',
